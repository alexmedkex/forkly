...
import { SBLCIssuedEventService } from './SBLCIssuedEventService'
...
describe('SBLCIssuedEventService', () => {
  let sblcIssuedEventService: ISBLCEventService
  let logger

  beforeEach(() => {
    sblcIssuedEventService = new SBLCIssuedEventService(
      sblcMockDataAgent,
      COMPANY_STATIC_ID,
      taskManagerMock,
      notificationManagerMock,
    ...
    logger = (sblcIssuedEventService as any).logger
    logger.info = jest.fn()
    logger.warn = jest.fn()
  ...
  describe('doEvent', () => {
    it('should call update', async () => {
      ...
      await sblcIssuedEventService.doEvent(sblc, decodedEventMock, rawEvent)
    ...
    it('should solve task for issuing bank', async () => {
      ...
      expect(documentManagerMock.shareDocument).toHaveBeenCalledTimes(1)
    })

    it('should solve task for issuing bank but not share document', async () => {
      taskManagerMock.getTasks = jest.fn().mockImplementationOnce(() => tasks)
      const sblc = buildFakeStandByLetterOfCredit({ transactionHash: TX_HASH, contractAddress: CONTRACT_ADDRESS })
      sblc.documentHash = ''
      sblc.issuingBankId = COMPANY_STATIC_ID
