import { LCContract } from '../blockchain/LCContract'
import { hashMessageWithCallData } from './HashFunctions'
import { ILCContract } from '../blockchain/ILCContract'
  ...
  describe('getCurrentState', () => {
    it('should get the current state', async () => {
      ...
      instance.methods.getCurrentStateId = jest.fn(() => {
        return { call: () => Web3Wrapper.web3Instance.utils.utf8ToHex('state1') }
      })

      const result = await lcContract.getCurrentState()

      expect(Web3Wrapper.web3Instance.utils.hexToAscii(result).replace(/\u0000/g, '')).toEqual('state1')
    })
