...
GIT_DIRTY="${GIT_DIRTY:-$(test -n "`git status --porcelain`" && echo "+CHANGES" || true)}"
...
gox \
    -osarch="${XC_OSARCH}" \
    -ldflags "-X gitlab.com/ConsenSys/client/uk/KomGo/${TOOL}/version.GitCommit='${GIT_COMMIT}${GIT_DIRTY}'" \
