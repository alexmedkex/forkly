...
import (
	...
	"gitlab.com/ConsenSys/client/uk/KomGo/komgo-member-vault/plugin-keymgmt/plugin/util"
)
...
func (b *backend) pathRotateCredentialsUpdate(ctx context.Context, req *logical.Request, _ *framework.FieldData) (string, error) {
	...
	newRsaKey, err := util.GenerateRSAKey(engineConf.KeymgmtConf.RSAKeyLength)
	if err != nil {
		return "nil", err
	}

	// Update the key remotely.
	// if err := b.client.UpdateRSAKey(engineConf.KeymgmtConf, engineConf.KeymgmtConf.RSAKeyLength, newRsaKey); err != nil {
	// 	return nil, err
	// }
