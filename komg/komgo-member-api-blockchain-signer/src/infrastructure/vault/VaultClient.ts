...
import IKeyData from './response/IKeyData'
...
export default class VaultClient {
  ...
  public async storeEthKey(ethKey: IPrivateKey) {
    const keyData = { key: ethKey }
    await this.storeKeyData(keyData)
  }

  public async readEthKey(): Promise<IPrivateKey> {
    const keyData = await this.readKeyData()
    return keyData.key
  }

  public async storeKeyData(keyData: IKeyData) {
    await this.writeKVSecret('eth-key', keyData)
  }

  public async readKeyData(): Promise<IKeyData> {
    const rawData = await this.readRawData()
    return rawData.data
  }
  public async readRawData(version?: number): Promise<KvReadResponse> {
    try {
      const response = await this.readKVSecret('eth-key', version)
      return response
    } catch (e) {
